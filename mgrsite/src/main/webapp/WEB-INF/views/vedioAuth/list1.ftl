		...
		$("#loginInfoDisplay").typeahead({
			...
			source:function(query,process){//加载数据源
				...
				&.ajax({
					...
					data:{keyword:query},
					success:function(data){//这个data就是一个json对象的数组
						if(data && data.length){
							process(data);//process就是交给我们获得数据之后用来调用的方法，这个方法执行了，下列列表就出来了
						}
					}
				});
			},
			   	...
			   	<div class="form-group">
				    ...
				    <div class="col-sm-6">
				    	<div class="dropdown" id="autocomplate">
					    	<input type="text" class="form-control" id="loginInfoDisplay" autocomplete="off" data-toggle="dropdown"/>
